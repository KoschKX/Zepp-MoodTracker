...
let _interpolationEnabled = false, _debugDayOffset = 0, _moodDataByDate = {}, _cachedDebugDate = null, _cachedDebugOffset = 0, _navDebounceTimer = null, _dateUpdateThrottle = null, _prevDisplayedMood = null, _prevDateStr = '', _graphNeedsRedraw = false, _isNavigating = false, _loadingText = null;
...
const setTodayMood = (v) => { _moodDataByDate[formatDateKey(getDebugDate())] = v; saveMoodData(); scheduleMoodHistorySave(); };
...
const updateUIAfterDateChange = (debugDateText, statusText, imgWidgets) => {
  const buildDateStr = (d, mode) => mode === 1 ? `${monthNames[d.getMonth()]} ${d.getFullYear()}` : `${d.getMonth() + 1}/${d.getDate()}/${d.getFullYear()}`;
  const updateMoodUI = () => {
...
Page({
  ...
  build() {
    initPX();
    const getDateStr = (m) => m === 1 ? `${monthNames[getDebugDate().getMonth()]} ${getDebugDate().getFullYear()}` : `${getDebugDate().getMonth() + 1}/${getDebugDate().getDate()}/${getDebugDate().getFullYear()}`;
    const todayMood = getTodayMood();
  ...
  onDestroy() { try { if (storageWriteTimeout) clearTimeout(storageWriteTimeout), storageWriteTimeout = null; saveMoodData(); const d = getItem(); if (d && d !== '{}') { try { syncToSettingsStorage(d); } catch {} try { sendDataToPhone(); } catch {} } } catch {} }
});
